...
class Enquiry extends Admin_Controller {
    ...
    public function index() {
        ...
        foreach ($enquiry_list as $key => $value) {
            ...
            $paid = $this->enquiry_model->getPaidByEnquiry($value["id"]);
            if(!empty( $paid)){
                $enquiry_list[$key]["paid_status"] = "paid";
            }else{
               $enquiry_list[$key]["paid_status"] = "unpaid"; 
            }
        }
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $inserted_id2 = $this->studentfeemaster_model->add_form($insert_array2);
