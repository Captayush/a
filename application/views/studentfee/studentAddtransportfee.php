                            ...
                            <table class="table table-striped table-bordered table-hover example table-fixed-header">
                                <thead class="header">
                                    <tr>
                                        ...
                                        ?>
                                        <th style="width: 10px">
                                            <!--<input type="checkbox" id="select_all" value="<?php if($fee->fees[0]->reciept_no==0){ }else{ echo $fee->fees[0]->reciept_no; } ?>"/>-->
                                        </th>
                                    ...
                                    foreach ($student_due_fee as $key => $fee) {
                                        ...
                                        foreach ($fee->fees as $fee_key => $fee_value) {
                                            ...
                                            if ($feetype_balance > 0 && strtotime($fee_value->due_date) < strtotime(date('Y-m-d'))) {
                                                ...
                                                ?>
                                                <td><input class="checkbox" type="checkbox" value="<?php if($fee_value->reciept_no==0){ }else{ echo $fee_value->reciept_no; } ?>" name="fee_checkbox" data-fee_master_id="<?php echo $fee_value->id ?>" data-fee_session_group_id="<?php echo $fee_value->fee_session_group_id ?>" data-fee_groups_feetype_id="<?php echo $fee_value->fee_groups_feetype_id ?>"></td>
                                                <td align="left"><?php
                                                    echo $fee_value->name;
                                                    ?></td>
...
<div id="listCollectionModal" class="modal fade">
    <div class="modal-dialog">
        <form action="<?php echo site_url('studentfee/addtransportfeegrp'); ?>" method="POST" id="collect_fee_group">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
...
<script type="text/javascript">
    ...
    $(".date_fee").datepicker({
        ...
        $(document).on('click', '.printDoc', function () {
            ...
            $.ajax({
                url: '<?php echo site_url("studentfee/printTransportFeesByName") ?>',
                type: 'post',
        ...
        $(document).on('click', '.printInv', function () {
            ...
            $.ajax({
                url: '<?php echo site_url("studentfee/printtransportFeesByGroup") ?>',
                type: 'post',
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#confirm-delete').on('click', '.btn-ok', function (e) {
            ...
            $.ajax({
                type: "post",
                url: '<?php echo site_url("studentfee/deletetransportpaidFee") ?>',
                dataType: 'JSON',
    ...
    $(document).ready(function () {
        $(document).on('click', '.printSelected', function () {
            ...
            if (array_to_print.length === 0) {
                ...
                $.ajax({
                    url: '<?php echo site_url("studentfee/printTransportFeesByGroupArray") ?>',
                    type: 'post',
        ...
        $(document).on('click', '.collectSelected', function () {
            ...
            $.ajax({
                type: 'POST',
                url: base_url + "studentfee/getcollecttransportfee",
